...
export const getTvPlans = async (tvProvider: string) => {
  try {
    const res = await fetch(`${URL}/v24u/tv/plans?provider=${tvProvider}`, {
      method: "GET",
      headers: buildHeaders(),
    });
    const data = await res.json();
...
export const verifyTvCardDetails = async ({
  provider,
  cardNumber,
}: {
  provider: string;
  ...
  try {
    const res = await fetch(`${URL}/v24u/tv/card/verify`, {
      method: "POST",
      body: JSON.stringify({ provider, cardNumber }),
      headers: buildHeaders(),
    });
    const data = await res.json();
